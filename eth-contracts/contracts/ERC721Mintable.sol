...
contract Ownable {
  ...
  function transferOwnership(address newOwner) public onlyOwner {
    ...
    // require(Address.isContract() == false, "new owner must be a real address"); // cjTodo
    _owner = newOwner;
    emit TransferOwnership(_owner);
  }
}
...
contract Pausable is Ownable {
  ...
  modifier whenNotPaused() {
    require(_paused == false, "must not be paused");
    _;
  }
  ...
  function setPaused(bool pause) public onlyOwner {
    _paused = pause;
    if (_paused) emit Paused(msg.sender); else emit Unpaused(msg.sender);
  }
}
...
//    @dev Approves another address to transfer the given token ID
  function approve(address to, uint256 tokenId) public {
    ...
    require(to != _tokenOwner[tokenId], "can not transfer token to self");
    // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
    require(msg.sender == _tokenOwner[tokenId] ||
      isApprovedForAll(_tokenOwner[tokenId], msg.sender),
      "not the token owner or an approved representative for the owner");
  ...
  function getApproved(uint256 tokenId) public view returns (address) {
    ...
    require(_tokenApprovals[tokenId] != address(0), "not approved");
  ...
  function setApprovalForAll(address to, bool approved) public {
    require(to != msg.sender, "can not set approval to self");
  ...
  function _transferFrom(address from, address to, uint256 tokenId) internal {
    ...
    require(_tokenOwner[tokenId] == from, "transfering from is not the owner");
    // TODO: require token is being transfered to valid address
    require(_checkOnERC721Received(from, to, tokenId, ""),
      "invalid address for transfer");
    // TODO: clear approval
...
contract ERC721Enumerable is ERC165, ERC721 {
  ...
  function tokenOfOwnerByIndex(address owner, uint256 index) public view
    ...
    require(index < balanceOf(owner),
      "owner has fewer tokens than the index");
  ...
  function tokenByIndex(uint256 index) public view returns (uint256) {
    require(index < totalSupply(), "index is greater than total supply");
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
  ...
  function tokenURI(uint256 tokenId) external view returns (string memory) {
      require(_exists(tokenId), "non existant tokenId");
